    ...
    <div class="flex flex-col xl:mb-0 xl:flex-row w-full">
      ...
      <div class="w-full px-1 xl:w-[75%] xl:ms-3">
        <div class="w-full bg-white rounded-md shadow-sm px-4">
          <div class="w-full flex justify-content-around items-center gap-7 xl:gap-4">
            <a
              ...
              <span class="text-[13px] xl:text-[14px]">
                Tất cả
              </span>
            </a>
            <a
        ...
        <div class="mt-3 w-full bg-white px-4 flex items-center text-red-700 text-[11px] xl:text-[13px] rounded-sm py-1">
          <i class="fa-solid fa-circle-exclamation"></i>
          <p class="ms-2 text-gray-500">Lưu ý: Tổng tiền đã bao gồm phí ship cho mỗi đơn hàng </p>
        </div>
        <div class="mt-3 w-full bg-white rounded-md shadow-sm xl:shadow-md px-4 order-content">
          <!-- Danh sách đơn hàng -->
          <!-- Một đơn hàng -->
            ...
            <?php foreach ($grouped_orders as $order) { ?>
              <div class="px-2 py-4 border-b border-orange-200">
                <div class="flex justify-between items-center mb-2">
                  <div class="text-[12px] xl:text-sm text-gray-500">Mã đơn: <span class="font-medium">#<?php echo $order['order_id'] ?></span></div>
                  <div class="text-[12px] xl:text-sm text-yellow-600 font-semibold"><?php echo $order['order_status'] ?></div>
                </div>
                  ...
                  <div class="flex justify-between items-center mb-4">
                    <div class="flex items-center space-x-4">
                      <img
                        ...
                        alt="product">
                      <div class="flex-1">
                        <p class="text-[12px] xl:text-[14px] font-medium">Sản phẩm: <?php echo $item['product_name'] ?></p>
                        <p class="text-[12px] xl:text-[14px] text-gray-500">Số lượng: <?php echo $item['quantity'] ?></p>
                      </div>
                    </div>
                ...
                <div class="text-right">
                  <p class="text-red-500 font-semibold xl:text-sm text-[14px]"><span class="text-[12px] xl:text-[14px] text-gray-400">Tổng tiền:</span> <?php echo Format::forMatPrice($order['total_price']) ?>đ</p>
                  <p class="text-[11px] xl:text-sm text-gray-600">Đặt lúc: <?php echo date('d/m/Y', strtotime($order['order_date'])) ?></p>
                </div>

                <div class="flex justify-end mt-4 space-x-2">
                  <button class="px-3 py-1 border rounded-md text-[12px] xl:text-sm text-gray-600 hover:bg-gray-100">
