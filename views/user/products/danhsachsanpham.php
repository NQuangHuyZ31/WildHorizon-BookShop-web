  ...
  <div class="flex flex-col px-2 xl:px-0 xl:flex-row">
    ...
    <div class="flex-1 min-h-80">
      <div class="mt-3 ps-4 bg-white py-3 rounded-md flex items-center">
        <p class="text-[14px] xl:text-[16px] uppercase font-bold text-black"><?php echo isset($keyword) ? $keyword : 'Tất cả sản phẩm' ?></p>
        <p class="text-blue-500 text-[12px] xl:text-[14px] ms-1">(
          <span class="product-count"><?php echo count($products) ?></span> sản phẩm)
        </p>
      </div>
      <?php if (!empty($products)) { ?>
        <div class="grid grid-cols-3 mt-2 gap-2 whr-product pb-3 xl:gap-4 xl:grid-cols-5 xl:mt-4">
        ...
        <?php if (count($products) > 10) { ?>
          <div class="mb-2">
            <button type="button" id="loadMore-product" class="load-more-product w-[100px] xl:w-full xl:text-sm text-[11px] xl:h-[40px] h-[35px] flex items-center justify-center" data-offset="30">Load more</button>
          </div>
        <?php } ?>
      <?php } else { ?>
