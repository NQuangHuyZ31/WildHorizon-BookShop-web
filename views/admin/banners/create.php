  ...
  <div class="flex min-h-screen">
    ...
    <div class="flex-1 p-8 overflow-auto">
      ...
      <div class="bg-white shadow-md rounded-md w-full p-6">
        <form action="<?php echo BASE_URL ?>/admin/banner/create" method="post" enctype="multipart/form-data">
          <input type="hidden" name="csrf_token" value="<?php echo $csrf_token ?>">
          <div class="w-full mb-5">
            <label class="w-full">
              <p class="text-[15px] font-semibold mb-2 ps-3">Mục tiêu quảng cáo</p>
              <input type="text" placeholder="Nhập mục tiêu quảng cáo" name="banner_name" class="input w-full <?php echo isset($error_msg['banner_name']) ? 'input-error' : 'input-info' ?>" />
            </label>
          ...
          <div class="w-full mb-5">
            ...
            <div class="flex items-center">
              <label class="flex items-center cursor-pointer">
                <input type="radio" name="status" value="active" class="radio radio-sm radio-info" checked="checked" />
                <span class="ms-3">Hoạt động</span>
              </label>
              <label class="flex items-center ms-10 cursor-pointer">
                <input type="radio" name="status" value="no_active" class="radio radio-sm radio-info" checked="checked" />
          ...
          <div class="w-full mb-5 flex justify-end">
            <button type="submit" class="btn btn-accent text-white" onclick="JsLoadingOverlay.show();">Thêm banner</button>
          </div>
        </form>
      </div>
    </div>
