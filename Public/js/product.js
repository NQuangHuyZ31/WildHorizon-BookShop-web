$(document).ready(function () {
  ...
  if (
    window.location.pathname == '/WildHorizon-BookShop/product' ||
    window.location.pathname.includes('/WildHorizon-BookShop/category/')
  ) {
    innitProductFilter();
  }
  ...
  function innitProductFilter() {
    ...
    if (from && to) {
      $(
        `.filter[data-from="${selectedFilters.price.from}"][data-to="${selectedFilters.price.to}"]`
      )
        .removeClass('price-unchecked')
        .addClass('price-checked');
    }

    if (supplier) {
      $(`#supplier-m-${selectedFilters.supplier}`)
        .removeClass('supplier-unchecked')
        .addClass('supplier-checked');
    }

    if (brand) {
      $(`#brand-m-${selectedFilters.brand}`)
        .removeClass('brand-unchecked')
        .addClass('brand-checked');
    }

    if (color) {
      $(`a[data-value="${selectedFilters.color}"]`)
        .removeClass('color-unchecked')
    ...
    $('.filter').click(function () {
      ...
      if (
        $this.hasClass('price-unchecked') ||
        $this.hasClass('price-checked')
      ) {
        filterType = 'price';
      } else if (
        $this.hasClass('supplier-unchecked') ||
        $this.hasClass('supplier-checked')
      ) {
        filterType = 'supplier';
      } else if (
        $this.hasClass('brand-unchecked') ||
        $this.hasClass('brand-checked')
      ) {
        filterType = 'brand';
      } else if (
        $this.hasClass('color-unchecked') ||
        $this.hasClass('color-checked')
      ) {
        filterType = 'color';
      }
      ...
      if (filterType) {
        $(`.${filterType}-checked`)
          .not($this)
          .removeClass(`${filterType}-checked`)
          .addClass(`${filterType}-unchecked`);
      }

      $this
        .toggleClass(`${filterType}-checked`)
        .toggleClass(`${filterType}-unchecked`);
      ...
      if (selectedFilters.price.from || selectedFilters.price.to) {
        urlParams.set(
          'price',
          selectedFilters.price.from + '-' + selectedFilters.price.to
        );
      } else {
        ...
        success: function (response) {
          if (response.success == 1) {
            ...
            response.products.forEach((product) => {
              $('.whr-product').append(`
                  <a href="${response.url}/product/${createSlug(
                product.product_name
              )}-${product.id}" class="mr-3 mb-4">
                    <div class="bg-white flex flex-col hover:shadow-md hover:rounded-sm whr-product-content">
                      <div class="whr-product-img py-2">
                        <img src="${response.url}/Public/upload/products/${
                product.product_image
              }" class="w-full h-full" alt="image">
                      </div>
                      <div class="px-2 mt-2">
                          ...
                          <div class="flex justify-between items-center">
                            <p class="flash-sale-product-price-sale ${
                              ...
                              product.discount_price > 0
                                ? ''
                                : 'hidden'
                            }"><s class="opacity-50">đ${new Intl.NumberFormat(
                'vi'
