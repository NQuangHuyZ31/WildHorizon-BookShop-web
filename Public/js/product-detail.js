$(document).ready(function () {
  ...
  $('#temp-address-province').change(() => {
    ...
    fetch('https://esgoo.net/api-tinhthanh/2/' + dataID + '.htm')
      ...
      .then(function (response) {
        $('#temp-address-district').html(
          '<option value="">Chọn quận/huyện</option>'
        );
        $('#temp-address-ward').html(
  ...
  $('.inc-quantity-product-detail').click(function () {
    ...
    $('.dec-quantity-product-detail').removeClass(
  ...
  $('.dec-quantity-product-detail').click(function () {
    ...
    $('.inc-quantity-product-detail').removeClass(
      'pointer-events-none opacity-75'
    );

    if (newValue == 1) {
      $('.dec-quantity-product-detail').addClass(
        'pointer-events-none opacity-75'
      );
    }
  });
  ...
  function checkQuantity(quantity, productID) {
    $.ajax({
      ...
      success: function (response) {
        if (response.error) {
          $('.inc-quantity-product-detail').addClass(
            'pointer-events-none opacity-75'
          );
