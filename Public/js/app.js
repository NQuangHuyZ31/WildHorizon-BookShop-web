            ...
            $('.whr-product').append(`
             <a href="${response.url}/product/${createSlug(
                      ...
                      ${
                        product.f_discount_pice > 0
                          ? new Intl.NumberFormat('vi').format(
                              product.price -
                                (product.price * product.f_discount_pice) / 100
                            )
                          : new Intl.NumberFormat('vi').format(
                              product.price -
              ...
              'vi'
                      ...
                      <img src="${
                        response.url
                      }/Public/images/icon/label-flashsale.svg" alt="icon_fs" width="70" height="40" class="mr-2 ${
              response.join_fs == 1 && product.f_quantity > 0 ? '' : 'hidden'
            }">
                    </div>
                  </div>
                    <div class="flex justify-end px-1 ${
                      response.join_fs == 1 && product.f_quantity > 0
                        ? ''
                        : 'hidden'
                    }">
                      <p class="text-gray-400" style="font-size: 11px;">còn ${
                        product.f_quantity
                      } sản phẩm</p>
                    </div>
                </div>
        ...
        if (response.data.length > 0) {
          response.data.forEach((product) => {
            $('.whr-product-flash-sale').append(`
              <a href="${response.url}/product/${createSlug(
              product.product_name
            )}-${product.product_id}" class="mr-2 bg-white mb-3">
            <div class="flex flex-col ">
              <div class="whr-product-img py-2">
                <img src="./Public/upload/products/${
                  product.product_image
                }" class="w-full h-full" alt="sanpham">
              </div>
              <div class="flash-sale-product mt-1 mx-2">
                <p class="text-sm flash-sale-product-title">${
                  product.product_name
                }</p>
                <div class="product-price-sale">
                  <p class="text-orange-500">${new Intl.NumberFormat(
                    'vi'
                  ).format(
                    product.price -
                      (product.price * product.discount_price) / 100
                  )}<u class="text-orange-500 ms-1">đ</u></p>
                  <p class="flash-sale-product-price-sale"><s class="opacity-50">đ${new Intl.NumberFormat(
                    'vi'
                  ).format(product.price)}</s>
                  <span class="text-white ms-2 bg-red-600 rounded-sm px-1">-${new Intl.NumberFormat(
                    'vi'
                  ).format(product.discount_price)}%</span></p>
                </div>
              </div>
            </div>
            <div class="flex justify-end px-1">
